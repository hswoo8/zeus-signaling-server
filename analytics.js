...
function renderAdminPage(snapshot) {
    ...
    const drainEnabled = deployment.draining === true;
    const staticMaintenance = deployment.staticMaintenance === true;
...
<style>
:root{color-scheme:light;--bg:#f4f5f2;--surface:#fff;--line:#d8ddd5;--text:#20231f;--muted:#697067;--green:#19764c;--orange:#c65d1b;--red:#b83b32;--ink:#38424b}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.45 system-ui,-apple-system,"Noto Sans KR",sans-serif;letter-spacing:0}header{background:#20231f;color:#fff;padding:18px 24px;border-bottom:4px solid var(--green)}header h1{font-size:20px;margin:0 0 4px}header p{margin:0;color:#cbd2c8;font-size:12px}main{max-width:1500px;margin:0 auto;padding:20px 24px 48px}.toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px}.toolbar a{color:#fff;background:var(--green);padding:8px 12px;border-radius:4px;text-decoration:none;font-weight:700}.channel-filter{display:inline-grid;grid-template-columns:repeat(4,minmax(70px,1fr));border:1px solid var(--line);border-radius:6px;overflow:hidden;margin-bottom:22px;background:var(--surface)}.channel-filter a{padding:9px 14px;text-align:center;text-decoration:none;color:var(--text);font-weight:700;border-right:1px solid var(--line)}.channel-filter a:last-child{border-right:0}.channel-filter a.current{background:var(--ink);color:#fff}.deployment-panel{display:flex;align-items:center;justify-content:space-between;gap:18px;background:var(--surface);border:1px solid var(--line);border-left:4px solid var(--green);border-radius:6px;padding:14px 16px;margin-bottom:18px}.deployment-panel.draining{border-left-color:var(--orange)}.deployment-panel strong,.deployment-panel small{display:block}.deployment-panel small{color:var(--muted);margin-top:3px}.deployment-panel button{border:0;border-radius:4px;background:var(--ink);color:#fff;padding:9px 13px;font-weight:700;cursor:pointer}.deployment-panel.draining button{background:var(--green)}.deployment-panel button:disabled{opacity:.45;cursor:default}.metrics{display:grid;grid-template-columns:repeat(5,minmax(130px,1fr));gap:10px;margin-bottom:22px}.metric{background:var(--surface);border:1px solid var(--line);border-radius:6px;padding:12px;min-height:92px}.metric span,.metric small{display:block;color:var(--muted)}.metric strong{display:block;font-size:26px;margin:5px 0;color:var(--green)}section{margin:24px 0}h2{font-size:15px;margin:0 0 9px;padding-left:9px;border-left:4px solid var(--orange)}.grid{display:grid;grid-template-columns:1fr 1fr;gap:20px}.chart-panel{background:var(--surface);border:1px solid var(--line);border-radius:6px;padding:12px;overflow:hidden}.legend{display:flex;justify-content:flex-end;gap:18px;color:var(--muted);font-size:12px}.legend span{display:flex;align-items:center;gap:6px}.legend i{width:18px;height:3px;display:inline-block}.legend .active{background:var(--ink)}.legend .launch,.bar-fill{background:var(--green)}.legend .single,.single-fill{background:var(--orange)}.legend .multi,.reason-fill{background:var(--red)}.line-chart{display:block;width:100%;height:auto;min-height:210px}.chart-grid{stroke:#e3e7e1;stroke-width:1}.axis-label{fill:#737b72;font-size:11px}.series{fill:none;stroke-width:3;stroke-linecap:round;stroke-linejoin:round}.active-line{stroke:var(--ink)}.launch-line{stroke:var(--green)}.single-line{stroke:var(--orange)}.multi-line{stroke:var(--red)}.bars{display:grid;gap:9px;min-height:180px}.bar-row{display:grid;grid-template-columns:minmax(72px,120px) minmax(100px,1fr) 44px;align-items:center;gap:9px}.bar-label{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#4d554c}.bar-track{height:12px;background:#e9ece7;border-radius:3px;overflow:hidden}.bar-track i{display:block;height:100%;border-radius:3px}.bar-row strong{text-align:right;font-variant-numeric:tabular-nums}.table-wrap{overflow:auto;background:var(--surface);border:1px solid var(--line);border-radius:6px}table{width:100%;border-collapse:collapse;min-width:560px}th,td{text-align:left;padding:9px 11px;border-bottom:1px solid #e7eae5;white-space:nowrap}th{position:sticky;top:0;background:#eef1ec;color:#4d554c;font-size:12px}tbody tr:hover{background:#fafbf9}.empty{text-align:center;color:var(--muted);padding:24px}.warn{color:var(--red);font-weight:700}.foot{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){main{padding:14px}.metrics{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}header{padding:14px}.deployment-panel{align-items:stretch;flex-direction:column}.legend{justify-content:flex-start;flex-wrap:wrap}.line-chart{min-width:720px}.chart-panel:has(.line-chart){overflow:auto}.channel-filter{display:grid}.channel-filter a{padding:9px 8px}}
</style>
</head>
<body>
...
<div class="deployment-panel ${drainEnabled ? 'draining' : ''}">
  ...
  <button id="deployment-drain" type="button" data-enable="${drainEnabled ? 'false' : 'true'}" ${staticMaintenance ? 'disabled' : ''}>${drainEnabled ? '신규 입장 재개' : '배포 드레인 시작'}</button>
</div>
<div class="metrics">
...
<script>const drainButton=document.getElementById('deployment-drain');if(drainButton&&!drainButton.disabled){drainButton.addEventListener('click',async()=>{const enabled=drainButton.dataset.enable==='true';const prompt=enabled?'신규 입장을 막고 진행 중인 대전이 끝나길 대기합니다. 드레인을 시작할까요?':'신규 대전 입장을 다시 허용할까요?';if(!confirm(prompt))return;drainButton.disabled=true;const response=await fetch('/admin/api/deployment/drain',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled})});if(!response.ok){drainButton.disabled=false;alert('배포 상태 변경에 실패했습니다.');return}location.reload()})}</script>
</body></html>`;
}
